...
public:
    SafetyStop() : Node("safety_stop_node")
    {
        ...
        decrease_speed_client_ = rclcpp_action::create_client<twist_mux_msgs::action::JoyTurbo>(
            this,
            "joy_turbo_decrease"
        );
        increase_speed_client_ = rclcpp_action::create_client<twist_mux_msgs::action::JoyTurbo>(
            this,
            "joy_turbo_increase"
        );
    ...
    void laserCallback(const sensor_msgs::msg::LaserScan & msg)
    {
        ...
        if (state_ != prev_state_) {
            ...
            if (state_ == State::WARNING) {
                ...
                RCLCPP_INFO(get_logger(), "WARNING: Obstacle detected within warning distance.");
            } else if (state_ == State::DANGER) {
                is_safety_stop.data = true;
                zones_.markers.at(0).color.a = 1.0;
                zones_.markers.at(1).color.a = 1.0;
                RCLCPP_INFO(get_logger(), "DANGER: Obstacle detected within danger distance. Stopping robot.");
                ...
                RCLCPP_INFO(get_logger(), "FREE: No obstacles nearby. Resuming normal speed.");
            }
